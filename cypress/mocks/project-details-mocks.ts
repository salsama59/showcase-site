...
export class ProjectDetailsMocks {
    ...
    public static getProjectDetailsList(url: string, response: ProjectDetail[]): void {
        HttpRequestUtils.interceptHttpGetRequest(url, response, HttpInterceptorAliasConstants.GET_PROJECT_DETAILS_LIST);
    };

    public static getProjectDetailsByProjectId(url: string, response: ProjectDetail[], projectId: string): void {
